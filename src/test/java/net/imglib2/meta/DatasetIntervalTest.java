...
public class DatasetIntervalTest {
	...
	private DatasetInterval<DoubleType, ?> dataset() {
		RandomAccessibleInterval<DoubleType> data = Data.intervalImage();
	...
	public void testExtension() {
		Dataset<DoubleType, ?> permuted = dataset().extend(RandomAccessibleIntervalView.Extension.border());
	...
	public void testExpansion() {
		long borderSize = 2;
		DatasetInterval<DoubleType, ?> permuted = dataset().expand(RandomAccessibleIntervalView.Extension.border(), 2, 2, 2, 2, 2);
	...
	public void testFluentMoveAxis() {
		DatasetInterval<DoubleType, ?> permuted = dataset().view().moveAxis(0, 3);
	...
	public void testFluentRotation() {
		DatasetInterval<DoubleType, ?> rotated = dataset().view().rotate(3, 2);
	...
	public void testFluentPermutation() {
		DatasetInterval<DoubleType, ?> permuted = dataset().view().permute(3, 2);
	...
	public void testFluentSlicing() {
		// Take a Z-slice
		DatasetInterval<DoubleType, ?> sliced = dataset().view().slice(2, 9);
	...
	public void testFluentConcatenation() {
		...
		DatasetInterval<DoubleType, ?> rotated = dataset().view().rotate(4, 2).slice(2, 9);
	...
	public void testFluentTranslation() {
		// Translate
		DatasetInterval<DoubleType, ?> translated = dataset().view().translate(-1, 0, 0, 0, 0);
	...
	public void testFluentInvertAxis() {
		// Invert Axis
		DatasetInterval<DoubleType, ?> inverted = dataset().view().invertAxis(1);
	...
	public void testFluentInverseTranslation() {
		// Translate
		DatasetInterval<DoubleType, ?> translated = dataset().view().translateInverse(1, 0, 0, 0, 0);
		Calibration calView = translated.store().info(Calibration.class);
		Assert.assertEquals(1.0, calView.axis(0).calibrated(0), 1e-6);
		// Translate & permute
	...
	public void testFluentSubsampling() {
		DatasetInterval<DoubleType, ?> translated = dataset().view().subsample(2, 1, 1, 1, 1);
		Calibration calView = translated.store().info(Calibration.class);
	...
	public void testFluentAddDimension() {
		...
		DatasetInterval<DoubleType, ?> translated = dataset().view().addDimension();
